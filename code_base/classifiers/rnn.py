...
class SentimentAnalysisRNN(object):
    ...
    def loss(self, wordvecs, labels, mask):
        ...
        ############################################################################
        rnn_out, rnn_cache = rnn_forward(wordvecs, h0,Wx, Wh, b)
        temp_affine_out, temp_affine_cache = temporal_affine_forward(rnn_out, W_a, b_a)
        ...
        dx_affine, dw_affine, db_affine = affine_backward(dscores, affine_cache)
        grads['W_fc'] = dw_affine
        grads['b_fc'] = db_affine
        
        davg = average_backward(dx_affine, avg_cache)
        
        dx_temp_affine, dw_temp_affine, db_temp_affine = temporal_affine_backward(davg, temp_affine_cache)
        grads['W_a'] = dw_temp_affine
        grads['b_a'] = db_temp_affine
        
        dx_rnn, dh0_rnn, dWx_rnn, dWh_rnn, db_rnn = rnn_backward(dx_temp_affine, rnn_cache)
        grads['Wx'] = dWx_rnn
        grads['Wh'] = dWh_rnn
        grads['b'] = db_rnn
        ############################################################################

...
def rnn_step_backward(dnext_h, cache):
    ...
    dforward = dnext_h * (1 - np.square(np.tanh(forward)))
...
def rnn_backward(dh, cache):
    ...
    dh_prev_i = np.zeros((N, H))

    for i in reversed(range(T)):
        dh_current = dh[i] + dh_prev_i
        dx_i, dh_prev_i, dWx_i, dWh_i, db_i= rnn_step_backward(dh_current, cache[i])
        dx[i] += dx_i
        db += db_i
        dWh += dWh_i
        dWx += dWx_i
